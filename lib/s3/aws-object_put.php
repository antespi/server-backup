...
// along with Server-Backup.  If not, see <http://www.gnu.org/licenses/>.
   ...
   function help_show($error = '', $errno = 0) {
      ...
      echo 'Usage : php ' . $argv[0] . ' <bucket> <filename> <localfile>' . PHP_EOL;
      echo '   bucket    - Bucket name' . PHP_EOL;
      echo '   filename  - AWS S3 filename (complete path, including parent folders if any)' . PHP_EOL;
      echo '   localfile - Local filename to upload' . PHP_EOL;
      exit($errno);
   }
   ...
   if (!empty($argv)){
      ...
      // Mandatory : Local file name
      if (!empty($argv[3])) $localfile = $argv[3];
      else help_show('No local file name specified', 3);
   }
